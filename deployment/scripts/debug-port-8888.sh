#!/bin/bash
# Debug script for port 8888 issues
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Debugging Port 8888 Issues${NC}"
echo -e "${BLUE}========================================${NC}"
...
test_simple_server() {
    echo -e "\n${BLUE}Testing simple HTTP server on port 8888...${NC}"
...
check_firewall() {
    ...
    if command -v iptables >/dev/null 2>&1; then
        echo -e "${BLUE}iptables rules for port 8888:${NC}"
        sudo iptables -L -n | grep -E "8888|ACCEPT.*0\.0\.0\.0" | head -10 || echo "  No specific rules for port 8888"
    fi

    # Check ufw
    if command -v ufw >/dev/null 2>&1; then
        echo -e "\n${BLUE}UFW status:${NC}"
        sudo ufw status | grep -E "8888|Anywhere" | head -10 || echo "  No UFW rules for port 8888"
    fi
}
...
echo -e "   ${YELLOW}lsof -ti :8888 | xargs kill -9${NC}"

echo -e "\n2. Start a minimal test server:"
echo -e "   ${YELLOW}python3 -m http.server 8888${NC}"

echo -e "\n3. Check for permission issues:"
echo -e "   ${YELLOW}sudo -u $(whoami) python3 -m http.server 8888${NC}"
